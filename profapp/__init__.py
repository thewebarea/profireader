...
def translate_():
    return "$scope.loading = true; $ok('', {}, function (resp) {$scope.loading = false; $scope.data = resp});  $scope._ = function (t, dict) { try { return $translate(t, dict ? dict : $scope) } catch (a) { return null }};"
...
def create_app(config='config.ProductionDevelopmentConfig'):
    ...
    app.jinja_env.globals.update(translate_=translate_)
