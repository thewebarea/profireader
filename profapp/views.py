...
def article():
    ...
    if form.validate_on_submit():
        article=Article(author_user_id=User.query.first().id,company_id=Company.query.first().id)
        article_history=ArticleHistory(form.article.data,0,User.query.first().id)
