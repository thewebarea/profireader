...
def materials(company_id):
    return render_template('company/materials.html', company_id=company_id,
...
@company_bp.route('/material_details/<string:company_id>/<string:article_id>/',
                  methods=['GET'])
@check_rights(simple_permissions(frozenset()))
@login_required
def material_details(company_id, article_id):
    return render_template('company/material_details.html', company_id=company_id, article_id=article_id)
...
def load_material_details(json, company_id, article_id):
    ...
    if article['portal_article'] and article['portal_article']['division']:
...
@check_rights(simple_permissions(frozenset()))
@ok
#TODO: VK by OZ:   fix search_for_company_to_join (this function now returns ALL companies)
...
#TODO: VK by OZ:   error  sqlalchemy.orm.exc.NoResultFound: No row was found for one()
#File "/home/oles/prof/profapp/models/company.py", line 232, in subscribe_to_company
