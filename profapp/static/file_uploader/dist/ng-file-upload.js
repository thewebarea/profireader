...
ngFileUpload.service('UploadBase', ['$http', '$q', '$timeout', function ($http, $q, $timeout) {
  ...
  this.upload = function (config) {
      ...
      if (isFile(file)) {
        ...
        if (config._start != null && resumeSupported) {
          ...
          if (config._chunkSize) {
            ...
            formData.append('chunkQuantity', Math.floor(config._file.size / config._chunkSize));
            formData.append('totalSize', config._file.size);
            formData.append('ftype', config._ftype);
            formData.append('upload_file_id', config._upload_file_id);
          }
        }
    ...
    config._upload_file_id = config.upload_file_id;
    config.headers = config.headers || {};
